...
const taskDraftStorageKey = 'glorytiktok-admin-task-draft';
const taskDraftOpenStorageKey = 'glorytiktok-admin-task-draft-open';

function readTaskDraft(): TaskForm {
  try {
    const saved = localStorage.getItem(taskDraftStorageKey);
    return saved ? { ...newTaskForm(), ...JSON.parse(saved) } : newTaskForm();
  } catch {
    return newTaskForm();
  }
}
...
function Admin({ onToast }: { onToast: (v: string) => void }) {
  const [showTask, setShowTask] = useState(() => sessionStorage.getItem(taskDraftOpenStorageKey) === 'true');
  const [taskForm, setTaskForm] = useState<TaskForm>(readTaskDraft);
  const update = (field: keyof TaskForm, value: string) => setTaskForm((current) => ({ ...current, [field]: value }));

  useEffect(() => {
    localStorage.setItem(taskDraftStorageKey, JSON.stringify(taskForm));
    sessionStorage.setItem(taskDraftOpenStorageKey, String(showTask));
  }, [taskForm, showTask]);
  const create = async (e: FormEvent) => {
    ...
    setShowTask(false); setTaskForm(newTaskForm()); localStorage.removeItem(taskDraftStorageKey); sessionStorage.removeItem(taskDraftOpenStorageKey); onToast('Görev oluşturuldu ve akışa eklendi.');
  };
